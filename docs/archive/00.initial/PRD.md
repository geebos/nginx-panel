...
- 每个待发布方案使用一个可原位更新的最新草稿；发布后版本冻结为不可变快照，发布行为可追溯。
...
- 保存一次完整的 Domain JSON 快照；Draft 可在发布前原位更新，已发布及历史版本不可变。
- 版本号在域名内单调递增，如 v1、v2。
- 当前没有 Draft 时，编辑保存产生下一版本号草稿；已有 Draft 时更新同一草稿 ID 和版本号，不直接改变线上配置。
- 发布成功后该版本成为当前发布版本。
...
- 保存按钮创建或更新当前草稿，不自动发布。
- 发布使用统一三步向导：Diff → Test → Publish；有未保存修改时必须先保存，不能直接发布浏览器本地状态。
...
- Overview、Routes、SSL、Headers、Advanced 共用“发布”操作；Logs、History 不显示发布入口。“更多”包含复制、停用/启用和删除。
- 当无草稿变更时，“发布”禁用，并解释原因；Domain Disabled、Runtime degraded 或存在冲突任务时同样禁用并给出处理入口。
...
- 发布 → 打开 Diff → Test → Publish 三步 Dialog；Test 成功且 checksum 仍匹配时才能创建发布任务并跳转 `/deployments/:deploymentId`。
...
Dialog 保存后创建下一版本号 Draft，或原位更新现有 Draft；Toast 区分“已创建 vN 草稿”“已更新 vN 草稿”“没有配置变化”，不自动发布。
...
- 顶部工具栏：日志类型多选（Access/Error，至少一项）、时间范围、HTTP Method、Status Code/范围、Path/关键字、Client IP，以及“查询”按钮；Method 支持独立清除。
- 工具栏右侧：“实时日志”Switch、“暂停/继续”“清屏”“滚动到底部”和“打开全局日志页”。
- 日志区使用等宽虚拟列表，每条记录独占一行；显示字段和顺序可配置，Domain Logs 与全局 Logs 的偏好分别保存在当前浏览器 `localStorage`，不改变 Nginx 采集格式。
...
- 历史筛选与实时筛选分别生效：实时关闭时修改控件只在点击“查询”后刷新历史；实时开启时合法筛选经 250 ms debounce 重连实时流，不刷新或清空历史，主按钮显示“刷新历史”并继续使用上一次历史筛选。
- 暂停只停止 UI 追加，连接保持且最多缓存 1,000 行；暂停时修改筛选会重连实时流并保持暂停。关闭实时模式会主动断开请求、清空暂停缓存并退出暂停。
- 断网后自动退避重连并携带 cursor；后端保证不漏掉已轮动但仍可读取的尾部，客户端按 `fileId + offset` 去重。
- 切换 Domain 或日志类型时立即终止旧流并切换数据源；普通筛选仅按上述历史/实时规则生效。
...
- 当前 Draft 未发布前，多次保存原位更新同一 ID 和版本号；当前不存在 Draft 时才创建下一版本号。
- Active、Superseded 以及任何曾被发布引用的版本永不修改；发布成功后当前 Draft 冻结，后续编辑创建下一版本号 Draft。
- Domain 的 enabled/disabled 是独立运行时投影，不属于 Config Version；Version 只描述 Domain 启用时应提供的业务配置。回滚或证书激活不得隐式改变 enabled 状态。
- 没有业务差异时不创建版本，也不更新 Draft。
...
- Given 合法 Proxy Route，When 保存，Then 当前 Draft 的 JSON 与 Nginx 预览均包含该 Route；已有 Draft 时 ID 和版本号保持不变。
...
- Given Draft 连续保存多次，When 尚未发布，Then始终只有一个 Draft、ID/版本号不变且 checksum/updatedAt 更新；发布后再次编辑才创建下一版本号。
- Given 打开发布向导，When依次完成 Diff 与 Test，Then只有 Test Deployment 成功且 Version/checksum 仍匹配时才能创建 Deploy Deployment；Draft 变化后旧 preflight 必须失效。
