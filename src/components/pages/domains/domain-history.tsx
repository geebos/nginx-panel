...
function domainIdFromPath(asPath: string) {
  return decodeURIComponent(asPath.match(/^\/domains\/([^/?]+)\/history/)?.[1] ?? "");
}
...
export function DomainHistory() {
  ...
  const [rollbackChanges, setRollbackChanges] = React.useState<Awaited<ReturnType<typeof getDomainVersionDiff>>["changes"] | null>(null);
    ...
    <>
      ...
      <div className="mx-auto w-full max-w-[1440px] px-4 py-6 md:px-8">
        ...
        {query.loading && !query.data ? <Skeleton className="h-80" /> : query.data?.versions.length ? (
          <div className="rounded-xl border border-border bg-card"><Table><TableHeader><TableRow><TableHead>Version</TableHead><TableHead>Status</TableHead><TableHead>Summary</TableHead><TableHead>Created</TableHead><TableHead>Updated</TableHead><TableHead>Checksum</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>
            {query.data.versions.map((version) => (
              <TableRow key={version.id}><TableCell className="font-mono">v{version.versionNumber}</TableCell><TableCell><StatusBadge status={version.id === domain?.activeVersion?.id ? "active" : version.status} /></TableCell><TableCell>{version.changeSummary}</TableCell><TableCell>{dateFormatter.format(version.createdAt)}</TableCell><TableCell>{dateFormatter.format(version.updatedAt)}</TableCell><TableCell className="max-w-40 truncate font-mono text-xs">{version.snapshotChecksum}</TableCell><TableCell><div className="flex justify-end gap-1"><Button size="icon-sm" variant="ghost" asChild><Link href={`/domains/${domainId}/versions/${version.id}`}><EyeIcon /><span className="sr-only">查看 v{version.versionNumber}</span></Link></Button>{compareBase && compareBase !== version.id ? <><Button size="icon-sm" variant="ghost" asChild><Link href={`/domains/${domainId}/versions/${version.id}/diff?base=${compareBase}`}><GitCompareArrowsIcon /><span className="sr-only">比较 v{version.versionNumber}</span></Link></Button>{version.status !== "draft" && version.status !== "failed" ? <Button size="icon-sm" variant="ghost" onClick={() => { setRollbackTarget(version); setRollbackChanges(null); setRollbackError(""); void getDomainVersionDiff(domainId, version.id, compareBase).then((result) => setRollbackChanges(result.changes)).catch((error: Error) => setRollbackError(error.message)); }}><RotateCcwIcon /><span className="sr-only">回滚到 v{version.versionNumber}</span></Button> : null}</> : null}</div></TableCell></TableRow>
            ))}
          </TableBody></Table></div>
      ...
      <AlertDialog open={Boolean(rollbackTarget)} onOpenChange={(open) => { if (!open && !rollingBack) setRollbackTarget(null); }}>
        <AlertDialogContent>
          ...
          <div className="rounded-lg border border-border bg-muted/30 p-3 text-sm">
            {rollbackError ? <p className="text-destructive">{rollbackError}</p> : rollbackChanges ? <p>检测到 {rollbackChanges.length} 项语义变化。确认后将创建 v{(query.data?.versions[0]?.versionNumber ?? 0) + 1} 和回滚任务。</p> : <p className="flex items-center gap-2 text-muted-foreground"><LoaderCircleIcon className="size-4 animate-spin" />正在计算与当前线上版本的差异…</p>}
          </div>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={rollingBack}>取消</AlertDialogCancel>
            <AlertDialogAction disabled={rollingBack || !rollbackChanges || Boolean(rollbackError)} onClick={(event) => { event.preventDefault(); if (!rollbackTarget) return; setRollingBack(true); setRollbackError(""); void rollbackDomainVersion(domainId, rollbackTarget.id).then((result) => router.push(`/deployments/${result.deploymentId}`)).catch((error: Error) => { setRollbackError(error.message); setRollingBack(false); }); }}>
