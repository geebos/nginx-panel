...
import { runConfigTest } from "@/worker/lib/deployment/config-test";
import { enqueueDiagnosticNginxTest, enqueueLogSettings, enqueuePublish, enqueueRebuildActive, enqueueReloadManagerTls } from "@/worker/lib/deployment/runner";
import { enqueueLogRotation } from "@/worker/lib/logs/rotator";
import type { RuntimeState } from "@/worker/lib/runtime/state";
...
export async function resumeQueuedDeployments(db: AppEnv["Variables"]["db"], runtime: RuntimeState) {
  ...
  for (const deployment of queued) {
    ...
    } else if (deployment.type === "diagnostic_test") {
      void enqueueDiagnosticNginxTest(db, deployment.id);
    } else {
